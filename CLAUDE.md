...
- **Server** (`src/`, compiled by tsc to `dist/`):
  ...
  - `store.ts` — better-sqlite3 (synchronous on purpose: check-and-set primitives are single statements). Three tables: `invite_codes` (single-use + faucet accrual), `spent_tokens` (SHA-256 hashes keyed by key epoch), `sessions` (points balances; `UPDATE … RETURNING` makes spend/top-up atomic).
...
- **Privacy:** never log or persist anything linking invite codes to blinded values, tokens, IPs, or user agents. The spent-set stores only token hashes.
- **Metering classes must stay in sync** across three places: the nginx static-exemption regex, the SW's `STATIC_RE`, and `MEDIA_EXT_RE`/`MEDIA_PREFIX_RE` in `server.ts` (media meters cheaper via `PP_POINTS_PER_MEDIA_REQUEST`, derived from `X-Original-URI`). Non-media static is exempt; images meter through the SW; video/audio bypass the SW and ride the pre-funded session.
